...
(function($) {
	...
	this.jwdfilepicker.init = function (options = {})
	{
		...
		// Body
		// Upload
		$dropzone = $('<form action="' + options.server_url + 'ajaxUploadFile?path=' + options.path + '" class="dropzone" id="file-dropzone">'+
		...
		var $meta_container = $('<div class="col-xs-12 col-md-5 col-lg-3 right-panel">' +
			...
			$.getJSON( options.server_url + 'index?path=' + options.path + '&ajax=true&filter_file=' + options.filter_file + '&id_file_picker=' + options.id_file, function (data) 
			{
				$loader.remove();
				item = data.data;
				empty = false; 
		...
		*/
		...
		jwdDropzone.on('error', function (file, response) {
			...
			message = 'message' in response ? response.message : 'cek console browser';
			$warning
		...
		jwdDropzone.on("success", function(file, response) 
		{
			...
			if (parsedResponse.status == 'error') {
				bootbox.alert('Error: ' + parsedResponse.message);
				return;
			}
