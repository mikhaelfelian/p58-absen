...
class JWDPDF extends \TCPDF {
	...
	public function Header() 
	{
		...
		$setting_result = $this->model->getSetting('app');
		foreach ($setting_result as $val) {
			$setting[$val['param']] = $val['value'];
		}
		
		$this->Image(ROOTPATH . 'public/images/' . $setting['logo_dokumen'], 10, 7, 10, 0, 'JPG', 'https://jagowebdev.com');
		
		$this->SetTextColor(30, 30, 30);
		$margin_left = 10;
		$margin_top = 7;
		$font_size = 10;
		$image_dim = getimagesize(ROOTPATH . 'public/images/' . $setting['logo_dokumen']);
