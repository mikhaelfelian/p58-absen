...
class Presensi_rekap extends \App\Controllers\BaseController
{
	...
	public function index() {
		...
		// Get user company assignments for company-specific time settings
		$userCompanyModel = new UserCompanyModel;
		$this->data['user_company_assignments'] = [];
		if (!empty($_GET['id_user'])) {
			$assignments = $userCompanyModel->getActiveCompanyByUser($_GET['id_user']);
			foreach ($assignments as $assignment) {
				$this->data['user_company_assignments'][$assignment->id_company] = $assignment;
			}
		}
		...
		$presensi = [];
		if (!empty($_GET['tahun'])) {
			$id_company = !empty($_GET['id_company']) ? $_GET['id_company'] : null;
			$result = $this->model->getPresensiByMonth($_GET['bulan'], $_GET['tahun'], $id_company);
			foreach ($result as $val) {
	...
	public function generateExcel($bulan, $tahun, $output) 
	{
		...
		$id_company = !empty($_GET['id_company']) ? $_GET['id_company'] : null;
		$result = $this->model->getPresensiByMonth($bulan, $tahun, $id_company);
