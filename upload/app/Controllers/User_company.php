...
class User_company extends BaseController
{
	...
	public function add() {
		...
		// Get all setting waktu presensi
		$settingWaktuPresensiModel = new SettingWaktuPresensiModel();
		$sql = 'SELECT * FROM setting_waktu_presensi ORDER BY nama_setting';
		$this->data['setting_waktu_presensi_list'] = $this->model->db->query($sql)->getResult();
	...
	public function store() 
	{
		...
		$idSettingWaktuPresensi = $request->getPost('id_setting_waktu_presensi');
		$shiftCutoffInput   = $request->getPost('shift_cutoff_time');
		$shiftCutoffTime    = $shiftCutoffInput ? $shiftCutoffInput : '05:00';
		if (strlen($shiftCutoffTime) === 5) {
			$shiftCutoffTime .= ':00';
		}
		$jamKerjaTarget = $request->getPost('jam_kerja_target');
		$jamKerjaTarget = !empty($jamKerjaTarget) ? intval($jamKerjaTarget) : 12;

		$data = [
			'id_user'          => $idUser,
			'id_company'       => $idCompany,
			'id_setting_waktu_presensi' => !empty($idSettingWaktuPresensi) ? $idSettingWaktuPresensi : null,
			...
			'shift_cutoff_time'=> $shiftCutoffTime,
			'jam_kerja_target' => $jamKerjaTarget,
		];
