...
class SendEmail
{
	...
	private function setStandard() 
	{
		...
		// Set port and secure from config if available
		if (isset($this->config->client['standard']['port'])) {
			$this->email->Port = $this->config->client['standard']['port'];
		}
		if (isset($this->config->client['standard']['secure'])) {
			$this->email->SMTPSecure = $this->config->client['standard']['secure'];
		}
	}
	...
	{ 
		try {
			...
			// Support single attachment (backward compatibility)
			...
			if (!empty($data['attachments']) && is_array($data['attachments'])) {
				foreach($data['attachments'] as $attachment) {
					if (isset($attachment['path']) && isset($attachment['name'])) {
						$this->email->AddAttachment($attachment['path'], $attachment['name']);
					}
				}
			}
