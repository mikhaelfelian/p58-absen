...
class SimpleQRCode
{
    ...
    public function generatePrintablePDF($patrol_points, $company_name = '')
    {
        ...
        foreach ($patrol_points as $patrol) {
            ...
            $nama_patrol = is_array($patrol) ? ($patrol['nama_patrol'] ?? '') : ($patrol->nama_patrol ?? '');
            $barcode = is_array($patrol) ? ($patrol['barcode'] ?? '') : ($patrol->barcode ?? '');
            
            // Patrol name
            $tcpdf->SetFont('helvetica', 'B', 10);
            $tcpdf->SetXY($x, $y);
            $tcpdf->Cell(80, 5, $nama_patrol, 0, 0, 'L');
            
            // Generate QR code image
            $qrImagePath = $this->generateQRCodeImage($barcode);
            if (file_exists($qrImagePath)) {
                ...
                $tcpdf->SetXY($x, $y + 8);
                $tcpdf->Cell(80, 40, $barcode, 1, 0, 'C');
            }
            
            // QR code text
            $tcpdf->SetFont('helvetica', '', 8);
            $tcpdf->SetXY($x, $y + 90);
            $tcpdf->Cell(80, 4, $barcode, 0, 0, 'C');
