...
class SettingAppModel extends \App\Models\BaseModel
{
	...
	public function saveData() 
	{
		...
		// Logo Register
		$logo_dokumen = $curr_db['logo_dokumen'];
		if ($_FILES['logo_dokumen']['name']) 
		{
			//old file
			if ($curr_db['logo_dokumen']) {
				if (file_exists($path . $curr_db['logo_dokumen'])) {
					$unlink = delete_file($path . $curr_db['logo_dokumen']);
					if (!$unlink) {
						$data['msg']['status'] = 'error';
						$data['msg']['content'] = 'Gagal menghapus gambar lama';
					}
			...
			$logo_dokumen = \upload_file($path, $_FILES['logo_dokumen']);
		}
		
		$data_db =[];
		if ($logo_login && $logo_app && $favicon && $logo_dokumen) 
		{
			...
			$data_db[] = ['type' => 'app', 'param' => 'logo_dokumen', 'value' => $logo_dokumen];
			$data_db[] = ['type' => 'app', 'param' => 'data_setelah_nama_user', 'value' => $_POST['data_setelah_nama_user']];
			$data_db[] = ['type' => 'app', 'param' => 'field_login', 'value' => $_POST['field_login']];
